...
namespace newres
{
    public partial class first_internal : System.Web.UI.Page
    {
        SqlConnection conn;
        SqlCommand cmd;
        SqlDataAdapter sda;
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                if (ddlexam.SelectedValue == "First Internal Exam")
                {
                    fn_ConnectDB();
                 ...
                 else  if (ddlexam.SelectedValue == "Second Internal Exam")
                {
                    fn_ConnectDB();
                    string query = "INSERT INTO second_internal_marks(reg_no,semester,subject_code,programme_id,max_marks,min_marks,obtained_marks,grade,result,class_id) Values(@reg_no,@semester,@subject_code,@programme_id,@max_marks,@min_marks,@obttained_marks,@grade,@result,@class_id)";
                    cmd = new SqlCommand(query, conn);
                    cmd.Parameters.AddWithValue("@reg_no", txtRegNo.Text);
                    cmd.Parameters.AddWithValue("@semester", txtsem.Text);
                    cmd.Parameters.AddWithValue("@subject_code", ddlSubjectCode.SelectedValue);
